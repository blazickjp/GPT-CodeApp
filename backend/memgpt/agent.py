...
def construct_system_with_memory(
    system, memory, memory_edit_timestamp, archival_memory=None, recall_memory=None
):
    full_system_message = "\n".join(
        [
            system,
...
class AgentAsync(object):
    ...
    def prepend_to_messages(self, added_messages):
        ...
        new_messages = (
            [self.messages[0]] + added_messages + self.messages[1:]
        )  # prepend (no system)
        self._messages = new_messages
    ...
    def rebuild_memory(self):
        ...
        curr_system_message = self.messages[
            0
        ...
        diff = united_diff(
            curr_system_message["content"], new_system_message["content"]
        )
    ...
    async def handle_ai_response(self, response_message):
        ...
        if response_message.get("function_call"):
            ...
            await self.interface.function_message(
    ...
    def verify_first_message_correctness(
        self, response, require_send_message=True, require_monologue=False
    ):
        ...
        if require_send_message and function_name != "send_message":
            printd(
                f"First message function call wasn't send_message: {response_message}"
            )
            return False
    ...
    async def step(
        ...
        try:
            ...
            if (
                ...
                printd(
                    f"WARNING: attempting to run ChatCompletion without user as the last message in the queue"
                )
            ...
            if not skip_verify and (
                ...
                printd(
                    f"This is the first message. Running extra verifier on AI response."
                )
                counter = 0
                while True:
            ...
            (
                ...
                heartbeat_request,
                function_failed,
            ) = await self.handle_ai_response(response_message)
            ...
            if current_total_tokens > MESSAGE_SUMMARY_WARNING_TOKENS:
                ...
                printd(
                    f"last response total_tokens ({current_total_tokens}) < {MESSAGE_SUMMARY_WARNING_TOKENS}"
                )
            ...
            if "maximum context length" in str(e):
                ...
                printd(
                    f"step() failed with openai.InvalidRequestError, but didn't recognize the error message: '{str(e)}'"
                )
                raise e
    ...
    async def summarize_messages_inplace(self, cutoff=None):
        if cutoff is None:
            ...
            for m in reversed(self.messages):
                ...
                cutoff -= 1
            cutoff = min(
                len(self.messages) - 3, cutoff
            )  # Always keep the last two messages too
        ...
        message_sequence_to_summarize = self.messages[
            1:cutoff
    ...
    async def free_step(self, user_message, limit=None):
        ...
        while limit is None or step_count < limit:
            if function_failed:
                ...
                new_messages, heartbeat_request, function_failed = await self.step(
                    user_message
                )
                step_count += 1
    ...
    async def archival_memory_search(self, query, count=5, page=0):
        results, total = await self.persistence_manager.archival_memory.search(
            query, count=count, start=page * count
        )
        num_pages = math.ceil(total / count) - 1  # 0 index
        if len(results) == 0:
