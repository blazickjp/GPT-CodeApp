...
async def prepare_archival_index_from_files_compute_embeddings(
    ...
    embedding_data = []
    for chunk in tqdm(
        archival_database, desc="Processing file chunks", total=len(archival_database)
    ):
        # for chunk in tqdm(f, desc=f"Embedding file {i+1}/{len(chunks_by_file)}", total=len(f), leave=False):
        try:
            embedding = await async_get_embedding_with_backoff(
                chunk["content"], model=embeddings_model
            )
        except Exception as e:
            print(chunk)
            raise e
