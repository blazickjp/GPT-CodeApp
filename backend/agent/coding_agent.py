...
class CodingAgent:
    ...
    def query(
        ...
        input = self.rewrite_input(input)
        logging.warning(f"Input: {input}")
    ...
    def rewrite_input(self, input: str) -> str:
        ...
        new_query = self.anthropic_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": self.generate_anthropic_prompt()},
                {
                    "role": "user",
                    "content": f"Rewrite the following question adding additional information to make the question more specific. Try to anticipate the user's intentions and respond with the query which will generate the most useful result for the user. <user-query>{input}</user-query>",
                },
            ],
            max_tokens=2000,
            temperature=0,
